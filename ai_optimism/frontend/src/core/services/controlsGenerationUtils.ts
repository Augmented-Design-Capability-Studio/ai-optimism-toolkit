...
/**
 * Transform expression syntax from variable.attributes[variable] to variable_attributes[variable]
 * This fixes expressions generated by AI that use the wrong syntax
 */
export function transformAttributeExpressions(expression: string, variableNames: string[]): string {
  let transformed = expression;
  
  for (const varName of variableNames) {
    // Transform: variable.attributes[variable] -> variable_attributes[variable]
    // Pattern: varName.attributes[varName] -> varName_attributes[varName]
    const pattern = new RegExp(`\\b${varName}\\.attributes\\[${varName}\\]`, 'g');
    transformed = transformed.replace(pattern, `${varName}_attributes[${varName}]`);
  }
